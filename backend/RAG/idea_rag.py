...
class IdeaRag(object):
    def __init__(self):
        ...
        self.describe_prompt_template = ChatPromptTemplate.from_messages(
            [
                ("system", """
                    你是资深的产品经理，你特别擅长将用户模糊的idea转化成初级产品描述。你需要基于用户的一个模糊的产品想法+历史对话信息，通过反问的形式逐步弄清用户需要做一款什么样的产品。
                    
                    规则：
                    1. 每次只提出【1个】问题，优先询问你对于用户最有疑问的点，围绕这个点进行提问，不要一次性问多个问题，不要问用户已经回答过的内容。
                    2. 当前疑问点彻底解决后开始提问下一个疑问点。
                    3. 当你已经很确定用户需要做一款什么样的产品了，不再提问，直接输出一段通顺完整且详细的初级产品描述。
                    4. 每次提问需要给用户举例的回答，如：“这个工具主要在什么业务场景下使用？比如1，日常编码记录，......、2，技术调研，......、3，项目复盘，......，4，其他？”
                    5. 你的任务不仅仅是通过反问弄清用户该产品的“目标用户是谁、业务场景、核心痛点、产品核心能力、使用终端、权限/业务约束”等，最重要的是将用户模糊的产品idea逐渐刻画清晰
...
if __name__ == '__main__':
    ...
    # 测试后端 RAG
    # res = IdeaRag().chain.invoke({"input": "我想做一个面向独立开发者的AI笔记工具"}, session_config)
    # print(res)
